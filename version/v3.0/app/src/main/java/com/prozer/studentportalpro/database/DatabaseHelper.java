...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public DatabaseHelper(Context context) {
        super(context, DB_NAME, null, 5); // Bumped to version 5 to add CampusExplorer table
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(
                "CREATE TABLE IF NOT EXISTS CampusExplorer(" +
                        "id INTEGER PRIMARY KEY AUTOINCREMENT," +
                        "buildingName TEXT," +
                        "latitude TEXT," +
                        "longitude TEXT," +
    ...
    public boolean isRegNoExists(String regNo) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_STUDENTS + " WHERE regNo = ?", new String[]{regNo});
    ...
    public boolean saveCampusLocation(String building,
                                      String latitude,
                                      String longitude,
                                      String date) {
        ...
        values.put("buildingName", building);
        values.put("latitude", latitude);
        values.put("longitude", longitude);
        values.put("dateCaptured", date);
    ...
    public boolean deleteCampusLocation(int id) {
    ...
    public Cursor searchCampusHistory(String keyword) {

        SQLiteDatabase db = this.getReadableDatabase();

        return db.rawQuery(

                "SELECT * FROM CampusExplorer WHERE buildingName LIKE ?",
    ...
    public ArrayList<CampusLocation> getCampusLocations() {
        ...
        Cursor cursor = db.rawQuery(
                "SELECT * FROM CampusExplorer ORDER BY id DESC",
                null);
